...
# 새로운 데이터가 처리 되었을 대 리랜더링하기

- Static Page 로 데이터 패칭, 데이터 캐싱을 했다.
- Static 된 내용을 리랜더링 하도록 요청하는 방법.

## 주의사항

- `오직 서버에서만 호출`할 수 있다.
- 클라이언트에서 호출해도 화면이 리랜더링 되서 내용 출력 안됨.
- 함수명이 고정이 되어 있음.
- `revalidatePath(인자)`

## `revalidataPath 방식 4가지`

- 특정 페이지만 내용 갱신 : revalidataPath(`/good/${id}`)
- 특정 경로의 모든 동적 페이지 내용 갱신 : revalidataPath(`/good/[id]`, "page")
- 특정 레이아웃을 갖는 모든 페이지 내용 갱신 : revalidataPath(`/(with-search)`, "layout")
- 전체 내용을 갱신 : revalidataPath(`/`, "layout")
- 특정 태그 값을 기준으로 데이터 내용 갱신 :

## `revalidataTag 방식`

- 특정 태그 값을 기준으로 데이터 내용 갱신 : revalidataTag(`good_${id}`)
...
```tsx
const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products/${id}`, {
  cache: "force-cache", ===> next: {tags: [`good_${id}`]}
});
```
...
- /src/actions/create-review-action.ts 를 변경
