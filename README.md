...
- Next 서버가 완성된 html 을 반환 > 만일, 클라이언트 컴포넌트가 있으면
- 클라이언트 컴포넌트만 번들링한 js를 돌려주고, 다시 하이드레이션 과정으로 진행

### Pages Router 에서는

- SSR (Server Side Rendering) : getServerSideProps 함수
- SSG (Static Site Generation) : getStaticProps 함수
- ISR (Incremental Static Regeneration) : SSG, revalidate 를 이용해 갱신
- 동적 라우터를 위해 필요한 함수(/good/1, /good/2..) : getStaticPaths 함수

#### 단점

- 위의 함수들은 무조건 라우터 경로에 맞는 페이지에만 작성 가능.
  - `http://localhost:3000/setting` ===> /src/pages/setting.tsx
- 일반 컴포넌트는 Props로 전달받는 방법 또는 Context 를 이용하는 방법 활용
...
- search 는 사용자가 뭘 검색할지 모르니 데이터 패칭은 부적합.
...
- Next.js 의 fetch 를 사용
- fetch("API", {cache 옵션})

## 종류

- `{ cache: "no-store" }` : 보관 ㄴㄴ
- `{ cache: "force-cache" }` : 무조건 보관 ㅇㅇ
...
- Static Page : Full Route Cashing 적용됨.
- Dynamic Page : 동적 생성

## Dynamic Page 의 경우

- 요청 마다 결과 내용이 다를 때 (Data Fetching 처리할 때)
...
## 동적 라우터 페이지를 Static Page 로 변경하기
