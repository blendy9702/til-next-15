...
- page 출력시 오래 걸리는 작업인 경우 loading 을 표현.
- Dynamic Page 에 활용하는데, 비동기 화면에 적용.

## Next.js 에서 제공하는 loading.tsx

- Page 에만 적용됨.
- 약속된 파일명

## 컴포넌트에서 loading을 처리하기 위해서 Suspense 를 활용.
...
- 각 라우터 마다 loading.tsx 를 설정하지 않으면 공통으로 사용됨.

## 컴포넌트에 세밀한 로딩처리

- React의 `Suspense` 로 처리
- loading.tsx 제거
...
- /src/components/all-goods.tsx 수정
- /src/components/random-goods.tsx 수정
...
export default function Home() {
  return (
    <div className={style.container}>
      <section>
        <h3>지금 추천하는 상품</h3>
        <Suspense fallback={<div>추천 상품 로딩중...</div>}>
          <RandomGoods />
        </Suspense>
      </section>
      <section>
        <h3>전체 상품</h3>
        <Suspense fallback={<div>전체 상품 로딩중...</div>}>
          <AllGoods />
        </Suspense>
      </section>
    </div>
